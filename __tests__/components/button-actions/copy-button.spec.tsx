...
function makeSut({ content = "" }: CopyButtonProps = {} as CopyButtonProps) {
...
describe("CopyButton", () => {
  ...
  it("should display an error toast when copying fails", async () => {
    ...
    await waitFor(() => {
      expect(toast.error).toHaveBeenCalledWith(`Erro ao copiar para a área de transferência: ${errorMessage}`);
    });
